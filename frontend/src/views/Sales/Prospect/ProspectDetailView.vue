...
<template>
  <section class="detail-page">
    ...
    <div v-else-if="!review" class="detail-empty">
      ...
      <RouterLink class="detail-empty-btn" to="/sales/pipeline"><i class="pi pi-arrow-left" /> Back to prospects</RouterLink>
    </div>
    ...
    <template v-else>
      ...
      >
          ...
          <ProspectComments v-else-if="expandedPanel === 'discussion'" class="expanded-content-card" :prospect-id="review.prospect.id" role="SALES_EXECUTIVE" :embedded="true" expanded />
...
.ai-shell > :deep(.tanya-card),
.ai-shell > :deep(.pc-wrap) {
  position: relative;
  z-index: 31;
}
.ai-shell > :deep(.tanya-input-row),
.ai-shell > :deep(.pc-input-row),
.ai-shell > :deep(.tanya-input-row button),
.ai-shell > :deep(.pc-send-btn) {
  position: relative;
  z-index: 32;
  pointer-events: auto;
}
