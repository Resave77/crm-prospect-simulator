...
const isCompanyEditRoute = computed(() => route.name === 'AdminCompanyEdit')
  ...
  section.addEventListener('click', (event) => {
    ...
    if (contentLines && contentLines.length >= 2) {
      ...
      if (buttons.length === 2) {
        ...
        contentLines[1].textContent = selected === 'Site' ? (form.address || '[Street Address]') : (form.companyAddress || '[Street Address]')
        ...
        contentLines[1].textContent = selected === 'Company' ? (form.companyAddress || '[Street Address]') : (form.address || '[Street Address]')
      }
    }
  })
}
...
async function handleSubmit() {
  ...
  try {
    if (isCompanyEditRoute.value) {
      await updateParentCompany(String(route.params.id), {
        name: form.name.trim(),
        termOfPayment: form.termPayment ? `${form.termPayment} Days` : '',
        npwpName: form.companyNpwpName.trim() || form.name.trim(),
        npwpAddress: form.companyNpwpAddress.trim() || form.companyAddress.trim(),
        npwpNumber: form.companyNpwpNumber.trim(),
        notes: form.notes.trim(),
        companyAddress: createAddress(
          form.companyAddress.trim(),
          form.companyProvince.trim(),
          form.companyDistrict.trim(),
          form.companyLatitude.trim(),
          form.companyLongitude.trim(),
        ),
...
onMounted(async () => {
  ...
  if (isCompanyEditRoute.value && route.params.id) {
    try {
      const company = await getParentCompany(String(route.params.id))
      form.name = company.name || ''
      form.parentCompanyName = company.name || ''
      form.parentCode = company.parentCode || ''
      form.companyAddress = company.address?.previewAddress || ''
      form.companyLatitude = company.address?.latitude == null ? '' : String(company.address.latitude)
      form.companyLongitude = company.address?.longitude == null ? '' : String(company.address.longitude)
      form.companyProvince = company.address?.province || ''
      form.companyDistrict = company.address?.district || ''
      form.companySubDistrict = company.address?.subDistrict || ''
      form.companyVillage = company.address?.village || ''
      form.companyNpwpName = company.npwpName || company.name || ''
      form.companyNpwpAddress = company.npwpAddress || company.address?.previewAddress || ''
      form.companyNpwpNumber = company.npwpNumber || ''
      form.termPayment = company.termOfPayment || form.termPayment
      form.notes = company.notes || ''
      form.companyContacts = (company.contacts?.length ? company.contacts : [blankContact()]).map((contact) => ({ name: contact.name || '', position: contact.position || '', phone: contact.phone || '', email: contact.email || '' }))
    } catch {
      optionsError.value = 'Unable to load company data. Please try again.'
    }
    return
  }
  if (route.params.id) {
    try {
      const detail = await getAdminCustomer(String(route.params.id))
      const customer = detail.customer as any
      const company = detail.parentCompany as any
      form.name = customer.name || ''
      form.customerSegment = customer.segment || ''
      form.customerCategory = customer.category || ''
      form.region = customer.region || ''
      form.salesExecutiveId = customer.salesExecutiveId || ''
      form.parentCompanyName = company.name || customer.parentCompanyName || ''
      form.parentCode = company.parentCode || customer.parentCode || ''
      form.address = customer.address?.previewAddress || ''
      form.latitude = customer.address?.latitude == null ? '' : String(customer.address.latitude)
      form.longitude = customer.address?.longitude == null ? '' : String(customer.address.longitude)
      form.province = [customer.address?.district, customer.address?.province].filter(Boolean).join(', ')
      form.district = customer.address?.subDistrict || ''
      form.subDistrict = customer.address?.subDistrict || ''
      form.village = customer.address?.village || ''
      form.companyAddress = company.address?.previewAddress || form.address
      form.companyLatitude = company.address?.latitude == null ? '' : String(company.address.latitude)
      form.companyLongitude = company.address?.longitude == null ? '' : String(company.address.longitude)
      form.companyProvince = [company.address?.district, company.address?.province].filter(Boolean).join(', ')
      form.companyDistrict = company.address?.subDistrict || ''
      form.companySubDistrict = company.address?.subDistrict || ''
      form.companyVillage = company.address?.village || ''
      form.contacts = customer.contacts?.length ? JSON.parse(JSON.stringify(customer.contacts)) : [blankContact()]
      form.companyContacts = company.contacts?.length ? JSON.parse(JSON.stringify(company.contacts)) : [blankContact()]
      form.pph = customer.ppn || form.pph
      form.idTkuNumber = customer.idTkuNumber || ''
      form.nik = customer.nik || ''
      form.companyNpwpName = company.npwpName || ''
      form.companyNpwpAddress = company.npwpAddress || ''
      form.companyNpwpNumber = company.npwpNumber || ''
        ...
        <div class="create-customer-header-inner mx-auto flex max-w-[1180px] flex-wrap items-center justify-between gap-[12px]">
          <div class="min-w-0 flex items-center gap-[12px]">
            <button
              ...
              @click="router.push(isCompanyEditRoute ? `/admin/companies/${route.params.id}` : '/admin/customers')"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-left size-[14px]"><path d="m12 19-7-7 7-7"></path><path d="M19 12H5"></path></svg>{{ isCompanyEditRoute ? 'Back to Company' : 'Back to Customer Site' }}
            </button>
            <div class="hidden h-[30px] w-px bg-[#e2e8f0] sm:block"></div>
            <div class="min-w-0">
              <h1 class="truncate font-['Inter'] text-[17px] font-bold leading-[22px] text-[#0f172a]">{{ isCompanyEditRoute ? 'Edit Company' : 'Edit Customer' }}</h1>
              <p class="truncate font-['Inter'] text-[11px] leading-[15px] text-[#94a3b8]">{{ isCompanyEditRoute ? 'Customer Management > Company > Edit' : 'Customer Management > Customer Site > Edit' }}</p>
            </div>
          </div>
          <div class="flex items-center gap-[8px]">
            ...
            >
              ...
              <svg v-else xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-pencil size-[14px]"><path d="M12 20h9"></path><path d="M16.376 3.622a1 1 0 1 1 3.002 3.002L7.368 18.635a2 2 0 0 1-.855.506l-2.872.838a.5.5 0 0 1-.62-.62l.838-2.872a2 2 0 0 1 .506-.854z"></path><path d="m15 5 3 3"></path></svg>Save Changes
            </button>
          </div>
        </div>
      </header>
      ...
      <div class="form-layout">
        ...
        <div class="form-stack">
          <div class="create-form-heading mb-[18px]">
            <h2 class="font-['Inter'] text-[24px] font-bold tracking-tight text-[#0f172a]">{{ isCompanyEditRoute ? 'Edit Company' : 'Edit Customer' }}</h2>
            <p class="mt-[5px] max-w-[760px] font-['Inter'] text-[13px] leading-[20px] text-[#64748b]">Review and update the customer location details without creating a new record.</p>
          </div>
            ...
            <div class="grid grid-cols-1 gap-[14px] px-[20px] py-[20px] md:grid-cols-2">
              <div class="overflow-visible rounded-[18px] border border-[#dbe3ef] bg-white shadow-[0px_10px_24px_0px_rgba(15,23,42,0.04)]">
                <div class="border-b border-[#eef2f7] bg-[#f8fafc] px-[16px] py-[14px]">
                  ...
                  <div v-if="isCompanyEditRoute" class="company-edit-info-box"><p>Information</p><p>This page is for editing company details only. Site identity is inherited from the company record and shown here for reference.</p></div>
                </div>
                <div class="space-y-[16px] p-[16px]">
                  ...
                  <div class="site-code-preview rounded-[18px] border border-[#e2e8f0] bg-[#f8fafc] p-[14px]">
              ...
              <div class="company-parent-wrapper overflow-hidden rounded-[18px] border border-[#dbe3ef] bg-white shadow-[0px_10px_24px_0px_rgba(15,23,42,0.04)]">
                <div class="border-b border-[#eef2f7] bg-[#f8fafc] px-[16px] py-[14px]">
                  <div class="flex flex-wrap items-center gap-[8px]">
          ...
          <!-- ADDRESS -->
          <section class="site-address-section overflow-visible rounded-[20px] border border-[#e2e8f0] bg-white shadow-[0px_14px_34px_0px_rgba(15,23,42,0.06)]">
            <div class="border-b border-[#eef2f7] px-[20px] py-[18px]">
              <div class="flex flex-wrap items-center gap-[8px]">
            ...
            <div class="grid grid-cols-1 gap-[14px] px-[20px] py-[20px] md:grid-cols-2">
              <div class="customer-site-contacts-card overflow-hidden rounded-[18px] border border-[#dbe3ef] bg-white shadow-[0px_10px_24px_0px_rgba(15,23,42,0.04)]">
                <div class="border-b border-[#eef2f7] bg-[#f8fafc] px-[16px] py-[14px]">
                  <div class="flex flex-wrap items-center gap-[8px]">
...
.company-edit-info-box{margin-top:14px;border:1px solid #cfe0ff;border-radius:16px;background:#eff6ff;padding:16px 18px}.company-edit-info-box p:first-child{margin:0;color:#0f172a;font:600 14px Inter,sans-serif}.company-edit-info-box p:last-child{margin:6px 0 0;max-width:760px;color:#334155;font:13px/20px Inter,sans-serif}
.form-layout:has(.company-edit-info-box) .tax-information-section,.form-layout:has(.company-edit-info-box) .other-master-data-section,.form-layout:has(.company-edit-info-box) .billing-shipment-section,.form-layout:has(.company-edit-info-box) .sales-assignment-section,.form-layout:has(.company-edit-info-box) .billing-delivery-fields{display:none!important}.form-layout:has(.company-edit-info-box) .customer-information-grid>.form-card:first-child .customer-info-input,.form-layout:has(.company-edit-info-box) .customer-information-grid>.form-card:first-child .customer-info-select{pointer-events:none;opacity:.72}
.form-layout:has(.company-edit-info-box) .customer-information-grid>.form-card:first-child>div:last-child{display:none!important}
.form-layout:has(.company-edit-info-box) .customer-info-input,.form-layout:has(.company-edit-info-box) .customer-info-select{display:none!important}.form-layout:has(.company-edit-info-box) label:has(.customer-info-input),.form-layout:has(.company-edit-info-box) label:has(.customer-info-select){display:none!important}.form-layout:has(.company-edit-info-box) .customer-info-input-search + *{display:none!important}
.form-layout:has(.company-edit-info-box) .customer-information-grid > div:nth-child(2) .customer-info-input,.form-layout:has(.company-edit-info-box) .customer-information-grid > div:nth-child(2) .customer-info-select{display:block!important}.form-layout:has(.company-edit-info-box) .customer-information-grid > div:nth-child(2) label:has(.customer-info-input),.form-layout:has(.company-edit-info-box) .customer-information-grid > div:nth-child(2) label:has(.customer-info-select){display:flex!important}
.form-layout:has(.company-edit-info-box) .customer-information-grid>div:first-child>div:nth-child(2){display:none!important}.form-layout:has(.company-edit-info-box) .customer-information-grid>div:first-child .site-code-preview{display:none!important}.form-layout:has(.company-edit-info-box) .customer-information-grid>div:nth-child(2) .customer-info-input,.form-layout:has(.company-edit-info-box) .customer-information-grid>div:nth-child(2) .customer-info-select{display:block!important}.form-layout:has(.company-edit-info-box) .customer-information-grid>div:nth-child(2) label:has(.customer-info-input),.form-layout:has(.company-edit-info-box) .customer-information-grid>div:nth-child(2) label:has(.customer-info-select){display:flex!important}
.form-layout:has(.company-edit-info-box) .customer-information-grid>div:first-child>div:has(.company-edit-info-box)~div{display:none!important}
.form-layout:has(.company-edit-info-box) div:has(> .site-code-preview){display:none!important}
.form-layout:has(.company-edit-info-box) .site-address-section>div:last-child,.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child>div:last-child{display:none!important}.form-layout:has(.company-edit-info-box) .site-address-section:after{content:'Information\A\AThis page is for editing company details only. Site address data is inherited from the company record and shown here for reference.';display:block;white-space:pre-wrap;margin:16px;border:1px solid #cfe0ff;border-radius:16px;background:#eff6ff;padding:16px 18px;color:#334155;font:13px/20px Inter,sans-serif}.form-layout:has(.company-edit-info-box) .site-address-section:after:first-line{font-weight:600;color:#0f172a}.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child:after{content:'Information\A\AThis page is for editing company details only. Site contacts are inherited from the customer site record and shown here for reference.';display:block;white-space:pre-wrap;margin:16px;border:1px solid #cfe0ff;border-radius:16px;background:#eff6ff;padding:16px 18px;color:#334155;font:13px/20px Inter,sans-serif}
.form-layout:has(.company-edit-info-box) .customer-info-input,.form-layout:has(.company-edit-info-box) .customer-info-select{display:block!important;opacity:1!important;pointer-events:auto!important}.form-layout:has(.company-edit-info-box) label:has(.customer-info-input),.form-layout:has(.company-edit-info-box) label:has(.customer-info-select){display:flex!important}
.form-layout:has(.company-edit-info-box) .company-parent-wrapper,.form-layout:has(.company-edit-info-box) .company-parent-wrapper *{visibility:visible!important}.form-layout:has(.company-edit-info-box) .company-parent-wrapper .customer-info-input,.form-layout:has(.company-edit-info-box) .company-parent-wrapper .customer-info-select{display:block!important;opacity:1!important;pointer-events:auto!important}.form-layout:has(.company-edit-info-box) .company-parent-wrapper label{display:flex!important}
.form-layout:has(.company-edit-info-box) .site-address-section>div:last-child,.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child>div:last-child{display:grid!important}.form-layout:has(.company-edit-info-box) .site-address-section>div:last-child>div:first-child,.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child{display:block!important}.form-layout:has(.company-edit-info-box) .site-address-section>div:last-child>div:first-child>div:last-child,.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child>div:first-child{display:none!important}
.form-layout:has(.company-edit-info-box) .site-address-section:after{display:none!important}.form-layout:has(.company-edit-info-box) .site-address-section>div:last-child>div:first-child:after{content:'Information\A\AThis page is for editing company details only. Site address data is inherited from the company record and shown here for reference.';display:block;white-space:pre-wrap;margin:16px;border:1px solid #cfe0ff;border-radius:16px;background:#eff6ff;padding:16px 18px;color:#334155;font:13px/20px Inter,sans-serif}
.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child>div:last-child{display:none!important}.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child:after{content:'Information\A\AThis page is for editing company details only. Customer site contact data is inherited from the customer site record and shown here for reference.';display:block;white-space:pre-wrap;margin:16px;border:1px solid #cfe0ff;border-radius:16px;background:#eff6ff;padding:16px 18px;color:#334155;font:13px/20px Inter,sans-serif}
.form-layout:has(.company-edit-info-box) .contacts-information-section:after,.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child:after{content:none!important;display:none!important}
.form-layout:has(.company-edit-info-box) .contacts-information-section *:after{content:none!important;display:none!important}
.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child:after{content:'Information\A\AThis page is for editing company details only. Customer site contact data is inherited from the customer site record and shown here for reference.';display:block;white-space:pre-wrap;margin:16px;border:1px solid #cfe0ff;border-radius:16px;background:#eff6ff;padding:16px 18px;color:#334155;font:13px/20px Inter,sans-serif}
.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child:after{content:none!important;display:none!important}
.form-layout:has(.company-edit-info-box) .customer-site-contacts-card:after{content:none!important;display:none!important}
.company-edit-contact-info{border:1px solid #cfe0ff;border-radius:16px;background:#eff6ff;padding:16px 18px;color:#334155;font:13px/20px Inter,sans-serif}.company-edit-contact-info p:first-child{margin:0;color:#0f172a;font-weight:600}.company-edit-contact-info p:last-child{margin:6px 0 0}
.form-layout:has(.company-edit-info-box) .customer-site-contacts-card>div:first-child{display:block!important;visibility:visible!important}
.form-layout:has(.company-edit-info-box) .customer-site-contacts-card>div:nth-child(2){display:block!important;visibility:visible!important}
.form-layout:has(.company-edit-info-box) .customer-site-contacts-card .company-edit-contact-info{display:block!important;visibility:visible!important}
.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child>div:first-child{display:block!important;visibility:visible!important}
.form-layout:has(.company-edit-info-box) .contacts-information-section>div>div:first-child>div:nth-child(2){display:block!important;visibility:visible!important}
</style>
