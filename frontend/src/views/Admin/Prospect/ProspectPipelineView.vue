...
onMounted(async () => {
  try {
    const [result] = await Promise.all([getSalesExecutives(), crm.loadPipeline()])
    sales.value = result
...
<template>
  <section class="admin-page">
    ...
    <div class="page-heading compact-heading">
      <div>
        <p class="eyebrow">Read-only sales funnel</p>
        <h1>Prospect Pipeline</h1>
        <p class="muted">Click any prospect card to open its ticketing &amp; comments.</p>
      </div>
      <Tag :value="`${filtered.length} records`" severity="secondary" />
    </div>
    ...
    <div class="filter-bar">
      <label>
        ...
        <Select v-model="salesFilter" :options="[{ id: '', fullName: 'All Sales Executives' }, ...sales]" option-label="fullName" option-value="id" />
      </label>
      <label>
        <span>Industry Group</span>
        <Select v-model="industryFilter" :options="industryOptions" option-label="label" option-value="value" />
      </label>
      <label>
    ...
    <div class="pipeline-board admin-pipeline-board">
      <section v-for="stage in visibleStages" :key="stage" class="pipeline-column">
        <header>
        ...
        <div class="pipeline-column-body">
          ...
          >
            ...
            <p><i class="pi pi-map-marker" /> {{ item.formattedAddress }}</p>
            <dl>
              <div><dt>Sales</dt><dd>{{ item.assignedSalesExecutive }}</dd></div>
              <div><dt>Stage</dt><dd>{{ item.status.replaceAll('_', ' ') }}</dd></div>
            </dl>
            <div class="kanban-card-footer">
              <span class="kanban-ticketing-link">
                <i class="pi pi-comments" /> Open ticketing
              </span>
            </div>
          </article>
...
.kanban-card { padding: 0.7rem; background: var(--surface-card); border: 1px solid var(--border-light); border-radius: var(--radius-md); box-shadow: var(--shadow-xs); transition: box-shadow var(--transition-fast), border-color var(--transition-fast); cursor: pointer; }
.kanban-card:hover { box-shadow: var(--shadow-sm); border-color: var(--brand-blue); }
.kanban-card:focus-visible { outline: 2px solid var(--brand-blue); outline-offset: 2px; }
...
.kanban-card-footer { display: flex; justify-content: flex-end; padding-top: 0.4rem; border-top: 1px solid #edf1f5; margin-top: 0.3rem; }
.kanban-ticketing-link { display: inline-flex; align-items: center; gap: 0.3rem; color: var(--brand-blue); font-size: 0.56rem; font-weight: 700; }
.kanban-ticketing-link i { font-size: 0.6rem; }
