...
import { formatErrorMessage, formatVisitDate, calcDuration } from '../../../utils/format'
...
<template>
  <section class="detail-page admin-detail-page">
    ...
    <div v-else-if="!review" class="detail-empty">
      ...
      <RouterLink class="detail-empty-btn" to="/admin/prospects"><i class="pi pi-arrow-left" /> Back to admin prospects</RouterLink>
    </div>
    ...
    <template v-else>
      ...
      <header class="prospect-hero">
        ...
        <div class="hero-row">
          ...
          <div class="hero-actions">
            ...
            <RouterLink
              v-if="canConvertProspect"
              class="hero-action hero-action-visit"
              :to="`/admin/prospects/${review.prospect.id}/convert`"
            >
              <i class="pi pi-arrow-right" /> Convert
            </RouterLink>
          </div>
      ...
      <div v-if="canConvertProspect" class="dcard dcard-active-visit">
        <div class="dcard-active-visit-row">
          <i class="pi pi-trophy" />
          <span>This prospect is <strong>WON</strong> and ready to convert into a customer.</span>
          <RouterLink class="dcard-active-visit-link" :to="`/admin/prospects/${review.prospect.id}/convert`">
            Convert now <i class="pi pi-arrow-right" />
          </RouterLink>
        </div>
      </div>
      ...
      <div class="detail-content-grid">
        ...
        <main class="detail-column detail-column-main">
          <section class="dcard dcard-prospect-core">
            ...
            <div class="prospect-detail-grid">
              ...
              <div class="detail-field detail-field-wide">
                <span><i class="pi pi-calendar-plus" /> Created</span>
                <strong>{{ new Date(review.prospect.createdAt).toLocaleString() }}</strong>
                <DataSourceBadge source="system" label="CRM" />
              </div>
              <div class="detail-field detail-field-wide">
                <span><i class="pi pi-map-marker" /> CRM Coordinates</span>
                <strong v-if="review.prospect.latitude != null && review.prospect.longitude != null">{{ review.prospect.latitude.toFixed(6) }}, {{ review.prospect.longitude.toFixed(6) }}</strong>
            ...
            <div v-if="review.visits.length" class="visit-grid">
              <article v-for="visit in displayedVisits" :key="visit.id" class="visit-card">
                ...
                <div class="visit-body">
                  <div class="visit-user"><i class="pi pi-user" /> Sales executive: {{ visit.salesExecutiveName }}</div>
                  <div><i class="pi pi-calendar" /> Check-in time: {{ new Date(visit.checkInAt).toLocaleString() }}</div>
                  <div v-if="visit.checkOutAt"><i class="pi pi-calendar-times" /> Check-out time: {{ new Date(visit.checkOutAt).toLocaleString() }}</div>
                  <div><i class="pi pi-sign-in" /> Check-in GPS: {{ visit.checkInLatitude.toFixed(6) }}, {{ visit.checkInLongitude.toFixed(6) }}</div>
                  <div v-if="visit.checkOutLatitude != null && visit.checkOutLongitude != null"><i class="pi pi-sign-out" /> Check-out GPS: {{ visit.checkOutLatitude.toFixed(6) }}, {{ visit.checkOutLongitude.toFixed(6) }}</div>
                  <div v-if="visit.checkOutAt"><i class="pi pi-clock" /> Duration: {{ calcDuration(visit.checkInAt, visit.checkOutAt) }}</div>
                  <div v-if="visit.visitResult"><i class="pi pi-flag" /> Result: {{ visit.visitResult }}</div>
                  <div v-if="visit.visitOutcome"><i class="pi pi-check-circle" /> Outcome: {{ visit.visitOutcome }}</div>
                  <div v-if="visit.visitNotes"><i class="pi pi-comment" /> Visit notes: {{ visit.visitNotes }}</div>
                  <div v-if="visit.followUpNotes"><i class="pi pi-directions" /> Follow-up: {{ visit.followUpNotes }}</div>
                  <img
                    v-if="visit.selfieReference && !visit.selfieReference.startsWith('SIMULATED')"
                    class="visit-selfie"
                    :src="visit.selfieReference.startsWith('/') ? `${apiBase}${visit.selfieReference}` : visit.selfieReference"
                    alt="Visit check-in selfie"
                  />
                  <div v-else class="visit-selfie-empty"><i class="pi pi-image" /> No check-in selfie</div>
                </div>
              </article>
            </div>
        ...
        <aside class="detail-column detail-column-side">
          ...
          <section v-if="canConvertProspect" class="dcard convert-card">
            ...
            <RouterLink class="hero-action hero-action-visit convert-link" :to="`/admin/prospects/${review.prospect.id}/convert`">
              Convert <i class="pi pi-arrow-right" />
            </RouterLink>
          </section>
        </aside>
      </div>
      ...
      <div class="detail-bottom-bar">
        ...
        <RouterLink v-if="canConvertProspect" class="dbar-btn dbar-checkin" :to="`/admin/prospects/${review.prospect.id}/convert`"><i class="pi pi-arrow-right" /> Convert</RouterLink>
        <span v-else class="dbar-btn dbar-checkin dbar-disabled"><i class="pi pi-check-circle" /> Review</span>
      </div>
