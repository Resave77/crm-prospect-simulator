...
<template>
  <section class="erp-dashboard-shell">
    ...
    <div class="rounded-[20px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_12px_40px_0px_rgba(15,23,42,0.05)] lg:p-[20px]">
      <div class="flex items-start gap-[12px]">
        <div class="flex size-[44px] items-center justify-center rounded-[14px] bg-[#eff6ff] text-[#2563eb]">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-[22px]">
        ...
        <div>
          <p class="font-['Inter'] text-[11px] font-bold uppercase tracking-[0.08em] text-[#94a3b8]">Overview</p>
          <h1 class="mt-[4px] font-['Inter'] text-[24px] font-bold text-[#0f172a]">Dashboard</h1>
          <p class="mt-[4px] max-w-[760px] font-['Inter'] text-[13px] leading-[20px] text-[#64748b]">Ringkasan cepat modul utama untuk navigasi harian, tanpa perlu berpindah jauh dari landing area.</p>
        </div>
      </div>
    </div>
    <div class="erp-stat-grid">
      <RouterLink to="/admin/customers" class="erp-stat-card rounded-[18px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_10px_30px_0px_rgba(15,23,42,0.04)]">
        <span class="font-['Inter'] text-[11px] font-bold uppercase tracking-[0.08em] text-[#94a3b8]">Total Pelanggan</span><strong class="mt-[8px] font-['Inter'] text-[28px] font-bold leading-[1] text-[#0f172a]">{{ crm.adminCustomers.length }}</strong>
      </RouterLink>
      <RouterLink to="/admin/visit-monitoring" class="erp-stat-card rounded-[18px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_10px_30px_0px_rgba(15,23,42,0.04)]">
        <span class="font-['Inter'] text-[11px] font-bold uppercase tracking-[0.08em] text-[#94a3b8]">Kunjungan Hari Ini</span><strong class="mt-[8px] font-['Inter'] text-[28px] font-bold leading-[1] text-[#0f172a]">{{ todayVisits.length }}</strong>
      </RouterLink>
      <RouterLink to="/admin/prospects/pipeline" class="erp-stat-card rounded-[18px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_10px_30px_0px_rgba(15,23,42,0.04)]">
        <span class="font-['Inter'] text-[11px] font-bold uppercase tracking-[0.08em] text-[#94a3b8]">Total Prospek</span><strong class="mt-[8px] font-['Inter'] text-[28px] font-bold leading-[1] text-[#0f172a]">{{ crm.pipeline.length }}</strong>
      </RouterLink>
      <RouterLink to="/admin/prospects/list" class="erp-stat-card rounded-[18px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_10px_30px_0px_rgba(15,23,42,0.04)]">
        <span class="font-['Inter'] text-[11px] font-bold uppercase tracking-[0.08em] text-[#94a3b8]">Prospek Berhasil</span><strong class="mt-[8px] font-['Inter'] text-[28px] font-bold leading-[1] text-[#0f172a]">{{ won }}</strong>
      </RouterLink>
    </div>
    <section class="quick-links rounded-[20px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_12px_40px_0px_rgba(15,23,42,0.05)] lg:p-[20px]"><div class="flex items-center justify-between gap-[12px]"><div><h2 class="font-['Inter'] text-[16px] font-bold text-[#0f172a]">Quick Links</h2><p class="mt-[2px] font-['Inter'] text-[12px] text-[#64748b]">Langsung lompat ke menu yang paling sering dipakai.</p></div><RouterLink class="inline-flex items-center gap-[8px] rounded-[10px] border border-[#e2e8f0] bg-white px-[14px] py-[10px] font-['Inter'] text-[13px] font-semibold text-[#475569] hover:bg-[#f8fafc]" to="/admin/dashboard">View All<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-[15px]"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg></RouterLink></div><div class="mt-[16px] grid gap-[12px] md:grid-cols-2 xl:grid-cols-4"><RouterLink v-for="item in [{ to: '/admin/customers', label: 'Customer Existing', icon: 'users' }, { to: '/admin/prospects/list', label: 'Customer Prospect', icon: 'list' }, { to: '/admin/prospects/pipeline', label: 'Prospect Pipeline', icon: 'chart' }, { to: '/admin/visit-monitoring', label: 'Visit Monitoring', icon: 'map' }]" :key="item.to" :to="item.to" class="group flex items-center justify-between rounded-[16px] border border-[#e2e8f0] bg-[#f8fafc] p-[14px] transition-all hover:border-[#cbd5e1] hover:bg-white"><div class="flex items-center gap-[10px]"><div class="flex size-[36px] items-center justify-center rounded-[12px] bg-[#fff1f2]"><i :class="`pi pi-${item.icon} text-[18px] text-[#dc2626]`" /></div><span class="font-['Inter'] text-[13px] font-semibold text-[#0f172a]">{{ item.label }}</span></div><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-[15px] text-[#94a3b8] transition-transform group-hover:translate-x-[2px]"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg></RouterLink></div></section>
    <div class="erp-bottom-grid"><section class="rounded-[20px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_12px_40px_0px_rgba(15,23,42,0.05)] lg:p-[20px]"><div class="flex items-center gap-[10px]"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-[18px] text-[#475569]"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"></path><path d="M14 2v4a2 2 0 0 0 2 2h4"></path><path d="M10 9H8"></path><path d="M16 13H8"></path><path d="M16 17H8"></path></svg><h2 class="font-['Inter'] text-[16px] font-bold text-[#0f172a]">Pipeline Activity</h2></div><div class="mt-[14px] space-y-[10px]"><div v-for="entry in stageCounts.slice(0, 4)" :key="entry.status" class="rounded-[14px] border border-[#eef2f7] bg-[#f8fafc] px-[14px] py-[12px] font-['Inter'] text-[13px] text-[#334155]"><span>{{ stageLabel(entry.status) }}</span><strong class="float-right">{{ entry.count }}</strong></div></div></section><section class="rounded-[20px] border border-[#e2e8f0] bg-white p-[16px] shadow-[0px_12px_40px_0px_rgba(15,23,42,0.05)] lg:p-[20px]"><div class="flex items-center gap-[10px]"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-[18px] text-[#475569]"><path d="M8 2v4"></path><path d="M16 2v4"></path><rect width="18" height="18" x="3" y="4" rx="2"></rect><path d="M3 10h18"></path></svg><h2 class="font-['Inter'] text-[16px] font-bold text-[#0f172a]">Recent Visits</h2></div><div class="mt-[14px] space-y-[10px]"><div v-for="item in todayAssignments" :key="item.id" class="flex justify-between rounded-[14px] border border-[#eef2f7] bg-[#f8fafc] px-[14px] py-[12px] font-['Inter'] text-[13px] text-[#334155]"><span>{{ item.customerName }}</span><strong>{{ new Date(item.checkInAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }}</strong></div><div v-if="!todayAssignments.length" class="rounded-[14px] border border-[#eef2f7] bg-[#f8fafc] px-[14px] py-[12px] font-['Inter'] text-[13px] text-[#334155]">Belum ada kunjungan hari ini</div></div></section></div>
  </section>
