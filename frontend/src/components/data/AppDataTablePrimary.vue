...
type SelectionControlStyle = "auto" | "checkbox" | "radio";
...
const props = withDefaults(
  defineProps<{
    ...
    maxHeight?: number;
    /** Suffix text appended to page size option labels (e.g. '/page') */
    pageSizeOptionSuffix?: string;
    ...
    /** Always show selection action bar when selection mode is enabled */
    selectionActionsAlwaysVisible?: boolean;
    /** Additional classes for the selection action bar */
    selectionActionsClass?: string;
    /** Selection action bar height. Numbers are treated as pixels. */
    selectionActionsHeight?: number | string;
    /** Show checkbox selection column on mobile/tablet too */
    selectionColumnOnMobile?: boolean;
    /** Selection control visual style. Single mode defaults to radio/circle unless set to checkbox. */
    selectionControlStyle?: SelectionControlStyle;
    ...
    /** Persist selection IDs in sessionStorage using columnControlsKey */
  ...
  }>(),
  {
    ...
    hideTopRightToolbar: false,
    initialPage: 1,
    initialPageSize: 25,
    maxHeight: undefined,
    pageSizeOptionSuffix: "",
    ...
    selectionActionsHeight: 44,
    selectionColumnOnMobile: false,
    selectionControlStyle: "auto",
...
const emit = defineEmits<{
  ...
  (e: "page-change", value: { page: number; rows: number }): void;
  (
...
const pageSizeOptions = computed(() => [
  ...resolvedPageSizePresets.value.map((value) => ({
    label: `${value}${props.pageSizeOptionSuffix}`,
    value,
  })),
  { label: "Custom", value: "custom" as const },
]);
const pageJumpOptions = computed(() =>
  Array.from({ length: totalPages.value || 1 }, (_, index) => {
    const value = index + 1;
    return {
      label: `${value}`,
      value,
...
const normalizeSelection = (value: unknown) => {
  ...
  return props.selectionMode === "single" ? normalized.slice(0, 1) : normalized;
};
...
const isCustomSingleCheckboxSelection = computed(
  () =>
    props.selectionMode === "single" &&
    props.selectionControlStyle === "checkbox"
);
const shouldShowSelectionActions = computed(
  () =>
    Boolean(props.selectionMode) &&
    (hasSelection.value || props.selectionActionsAlwaysVisible)
);
const selectionActionsStyle = computed(() => ({
  height:
    typeof props.selectionActionsHeight === "number"
      ? `${props.selectionActionsHeight}px`
      : props.selectionActionsHeight,
}));
...
const selectionStorageKey = computed(() =>
  props.selectionPersistence && props.columnControlsKey
...
const persistSelectionToStorage = (nextSelection: unknown[]) => {
  ...
  const compactSelectionValues = normalizeSelection(nextSelection);
  const compactSelectionValueIds = Array.from(
    new Map(
      compactSelectionValues
        .map((item) => {
  ...
  if (compactSelectionValueIds.length === 0) {
  ...
  window.sessionStorage.setItem(
    storageKey,
    JSON.stringify(compactSelectionValueIds)
  );
};
...
const hydrateSelectionFromStorage = () => {
  ...
  try {
    ...
    const compactSelectionValues = normalizeSelection(
      Array.from(
        new Map(
          parsed
            .map((value) => {
...
const onPageChange = (event: PageState) => {
  ...
  emit("page-change", { page: nextPage, rows });
...
<template>
  ...
  >
    <div v-if="hasPermission" class="data-table-card">
      <div
        v-if="!props.hideTopRightToolbar || !props.hidePaginationControls"
      ...
      >
        ...
        <div
          v-if="!props.hideTopRightToolbar"
      ...
      <div
        ...
        :class="[
          'data-table__selection flex shrink-0 animate-in items-center justify-between gap-3 slide-in-from-top duration-300',
        ...
        >
          ...
          >
            ...
            <Column
              v-if="
                selectionMode &&
                !withoutCheckBoxSelection &&
                !isCustomSingleCheckboxSelection
              "
              :selection-mode="selectionMode"
              :class="
                props.selectionColumnOnMobile
                  ? 'data-table__selection-column'
                  : 'data-table__selection-column hidden md:table-cell'
              "
              :style="{ width: '50px', minWidth: '50px', maxWidth: '50px' }"
            />
            <Column
              v-if="
                isCustomSingleCheckboxSelection && !withoutCheckBoxSelection
              "
              :class="
                props.selectionColumnOnMobile
                  ? 'data-table__selection-column'
            ...
            >
              <template #body="{ data }">
                <button
                  type="button"
                  class="inline-flex size-[18px] items-center justify-center rounded-[4px] border transition-colors"
                  :class="
                    isItemSelected(data)
                      ? 'border-[#2563eb] bg-[#2563eb] text-white'
                      : 'border-[#cbd5e1] bg-white text-transparent hover:border-[#94a3b8]'
                  "
                  @click.stop="toggleItemSelection(data)"
                >
...
.data-table__toolbar :deep(.p-paginator-current) {
  ...
  background: transparent;
  padding: 0 0.2rem;
  font-size: calc(var(--font-table) * 0.8);
  font-weight: 600;
  color: #64748b;
}
...
:global(.p-select-overlay .p-select-item) {
