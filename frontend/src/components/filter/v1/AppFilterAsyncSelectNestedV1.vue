...
interface Props {
  ...
  placeholder?: string;
  selectedLabelResolver?: (value: number) => string | null | undefined;
...
const isResetOption = (value: OptionValue | null) =>
  value === RESET_OPTION_VALUE;
...
const queryKey = computed(
  () => ["filter-shortlist-nested", props.cacheKey, state.search] as const
);
...
const loadedOptions = computed(() =>
  items.value.map((item: ShortListItem) => {
    ...
    const subLabel = props.subLabelKey ? (raw[props.subLabelKey] ?? "") : "";
    const value = (raw[props.idKey] ?? raw["id"]) as number;

    return { label: String(label), subLabel: String(subLabel), value };
  })
);
...
const resolveModelValue = (val: OptionValue | OptionValue[] | null) => {
  if (Array.isArray(val)) {
    return val.includes(RESET_OPTION_VALUE)
      ? []
      : (val.filter(
          (item): item is number => typeof item === "number"
        ) as number[]);
  }
  return val === RESET_OPTION_VALUE ? null : (val as number | null);
};

const internalValue = computed<OptionValue | OptionValue[] | null>({
...
const isSelected = (val: OptionValue | null) => {
  if (isResetOption(val)) return false;

  if (!props.modelValue || typeof val !== "number") return false;
  if (Array.isArray(props.modelValue)) return props.modelValue.includes(val);
  return props.modelValue === val;
};

const toggleOption = (optVal: OptionValue) => {
  ...
  if (typeof optVal !== "number") return;
...
const resolveSelectedLabel = (value: OptionValue) =>
  loadedOptions.value.find((option) => option.value === value)?.label ??
  (typeof value === "number" ? props.selectedLabelResolver?.(value) : null) ??
  null;
  ...
  <template v-if="isMobile && mobileModal">
    ...
    >
      <span
        ...
        :class="hasSelection ? 'text-[#0f172a]' : 'text-[#94a3b8]'"
      >
        {{ displayLabel }}
      </span>
      <span
...
:deep(.p-select-item--selected) .nested-option__radio,
:deep(.p-multiselect-item--selected) .nested-option__radio {
  border-color: #dc2626;
...
:deep(.p-select-item--selected) .nested-option__radio-dot,
:deep(.p-multiselect-item--selected) .nested-option__radio-dot {
...
:deep(.p-select-item--selected) .nested-option__label,
:deep(.p-multiselect-item--selected) .nested-option__label {
  color: #dc2626;
  font-weight: 600;
}
...
:deep(.p-select-item--selected) .nested-option__sublabel,
:deep(.p-multiselect-item--selected) .nested-option__sublabel {
  color: #dc2626;
}

:deep(.p-select-option-check-icon),
:deep(.p-multiselect-option-check-icon) {
  display: none !important;
}
